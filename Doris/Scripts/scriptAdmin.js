...
$("[data-tour=parentTour]").on("change", function () {
    var parentId = $(this).val();
    $.getJSON("/ProductVcms/GetChildCategory", { parentId: parentId }, function (data) {
        var items = [];
        items.push("<option value>Chọn danh mục</option>");
        $.each(data, function (key, val) {
            items.push("<option value='" + val.Id + "'>" + val.CategoryName + "</option>");
        });
        $("[data-tour=categoryTour]").html(items.join(""));
    });

    $.getJSON("/ProductVcms/GetTour", { categoryId: parentId }, function (data) {
        var items = [];
        items.push("<option value>Chọn Tour</option>");
        $.each(data, function (key, val) {
            items.push("<option value='" + val.Id + "'>" + val.Name + "</option>");
        });
        $("[data-tour=tour]").html(items.join(""));
    });
});

$("[data-tour=categoryTour]").on("change", function () {
    var catId = $(this).val();
    $.getJSON("/ProductVcms/GetTour", { categoryId: catId }, function (data) {
        var items = [];
        items.push("<option value>Chọn Tour</option>");
        $.each(data, function (key, val) {
            items.push("<option value='" + val.Id + "'>" + val.Name + "</option>");
